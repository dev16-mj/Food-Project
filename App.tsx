...
type AppTab = 'browse' | 'donate' | 'requests' | 'ngos' | 'stats' | 'profile';
...
const App: React.FC = () => {
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'requests': return <MyRequests requests={requests} role={currentUser.role} />;
    ...
    <div className="min-h-screen bg-white">
      ...
      <header className="border-b border-gray-100 bg-white sticky top-0 z-50">
        ...
        <div className="max-w-7xl mx-auto px-6 flex items-center">
          ...
          <NavTab icon="fa-chart-simple" label="Waste Statistics" active={activeTab === 'stats'} onClick={() => setActiveTab('stats')} />
        </div>
      </header>
...
const NavTab: React.FC<{ icon: string, label: string, active: boolean, onClick: () => void }> = ({ icon, label, active, onClick }) => (
  <button 
    onClick={onClick}
    className={`flex items-center gap-2 px-6 py-4 border-b-2 transition-all text-sm font-medium ${
      active 
